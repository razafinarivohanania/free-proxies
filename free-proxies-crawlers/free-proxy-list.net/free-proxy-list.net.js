...
(async () => {
    ...
    const response = await API.Connection.getConnection('https://free-proxy-list.net/', configuration.proxy, API.Connection.RESPONSE_TYPE.document);
    ...
    for (const proxyElement of proxyElements) {
        ...
        if (proxy) {
            const url = `${configuration.rest.addProxy}?ip=${proxy.ip}&port=${proxy.port}&country=${proxy.country}&provider=${proxy.provider}`;
